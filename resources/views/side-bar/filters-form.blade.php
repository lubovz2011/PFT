        ...
        <div class="form-group col">
            ...
            <select class="form-control" name="filter-times">
                ...
                @foreach(\App\Classes\Utils\DataSets::getDateOptions() as $key => $option)
                    <option value="{{$key}}"
                            @if(request()->get('filter-times') == $key) selected @endif>
                        {{$option}}
                    </option>
                @endforeach
            </select>
        ...
        <div class="form-group col">
            ...
            <select class="form-control" name="filter-types">
                ...
                @foreach(\App\Classes\Utils\DataSets::getTypeOptions() as $key => $option)
                    <option value="{{$key}}"
                            @if(request()->get('filter-types') == $key) selected @endif>
                        {{$option}}
                    </option>
                @endforeach
            </select>
            ...
            <select class="form-control" name="filter-accounts[]" multiple>
                ...
                @foreach($accounts as $account)
                    <option value="{{$account->id}}"
                            @if(in_array($account->id, request()->get('filter-accounts') ?? [])) selected @endif>
                        {{$account->title}}
                    </option>
                @endforeach
            </select>
                ...
                @foreach($categories as $category)
                    <optgroup label="{{$category->name}}">
                        <option value="{{$category->id}}"
                                @if(in_array($category->id, request()->get('filter-categories') ?? [])) selected @endif>
                            {{$category->name}}
                        </option>
                        @foreach($category->categories as $subCategory)
                            <option value="{{$subCategory->id}}"
                                    @if(in_array($subCategory->id, request()->get('filter-categories') ?? [])) selected @endif>
                                {{$subCategory->name}}
                            </option>
                        @endforeach
                    </optgroup>
